  ...
  return (
    <tbody className='even:tw-bg-slate-50 hover:tw-bg-slate-100'>
      ...
      <tr className='tw-border-0 tw-border-b-2 tw-border-solid tw-border-slate-200 hover:tw-bg-slate-100'>
        ...
        {loser.sets?.map((set, index) => { return <td className='tw-text-left tw-pt-1 phone:tw-hidden' key={index}>{set > winner.sets[index] ? (<strong>{set}</strong>) : (<span className='tw-text-neutral-600'>{set}</span>)}</td> })}
      </tr>
    </tbody>
  )
}